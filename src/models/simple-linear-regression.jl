...
function estimate_simple_transcript_linear_regression(input)
    ...
    qx_log_sigma_param = tf.Variable(tf.fill([n], 0.0f0), name="qx_log_sigma_param")
    qx_log_sigma_map = edmodels.PointMass(params=qx_log_sigma_param)

    inference = ed.MAP(Dict(w => qw_map, x_log_sigma => qx_log_sigma_map), data=datadict)
    ...
    error_sigma = exp.(sess[:run](qx_log_sigma_param))
