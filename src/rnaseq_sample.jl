...
function populate_ts_metadata!(ts, ts_metadata, gene_annotations, gene_pattern_str)
    ...
    elseif gene_pattern_str !== nothing
        ...
        unknown_gene_num = 1
        for t in ts
            mat = match(gene_pattern, t.metadata.name)
            if mat !== nothing
                gid = isempty(mat.captures) ? mat.match : mat.captures[1]
                ts_metadata.gene_id[t.metadata.name] = gid
            else
                gid = string("unknown-gene-", unknown_gene_num)
                unknown_gene_num += 1
                ts_metadata.gene_id[t.metadata.name] = gid
            end
        end
